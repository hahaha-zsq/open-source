...
![image-20220715141705004](image/MySQL(5)日志与备份篇.assets/image-20220715141705004.png)
...
* **二进制日志**：记录所有更改数据的语句，可以用于**主从服务器之间的数据同步**，以及服务器**遇到故障**时数据的无损失恢复。 
* **中继日志**：用于主从服务器架构中，从服务器用来**存放主服务器二进制日志内容的一个中间文件**。 从服务器通过读取中继日志的内容，来同步主服务器上的操作。 
...
![image-20220715145650406](image/MySQL(5)日志与备份篇.assets/image-20220715145650406.png)
...
![image-20220715155010381](image/MySQL(5)日志与备份篇.assets/image-20220715155010381.png)
...
![image-20220715160249271](image/MySQL(5)日志与备份篇.assets/image-20220715160249271.png)
...
![image-20220715160657093](image/MySQL(5)日志与备份篇.assets/image-20220715160657093.png)
...
* 第二步：重建日志
  ...
  ![image-20220715161132368](image/MySQL(5)日志与备份篇.assets/image-20220715161132368.png)
...
![image-20220715161216556](image/MySQL(5)日志与备份篇.assets/image-20220715161216556.png)

### 4.4 MySQL 8.0 新特性

![image-20220715161321565](image/MySQL(5)日志与备份篇.assets/image-20220715161321565.png)
...
binlog主要应用场景：
  ...
  ![image-20220715161800635](image/MySQL(5)日志与备份篇.assets/image-20220715161800635.png)

![image-20220715161842703](image/MySQL(5)日志与备份篇.assets/image-20220715161842703.png)
...
![image-20220715163520596](image/MySQL(5)日志与备份篇.assets/image-20220715163520596.png)
...
![image-20220715163811664](image/MySQL(5)日志与备份篇.assets/image-20220715163811664.png)
...
![image-20220715164107352](image/MySQL(5)日志与备份篇.assets/image-20220715164107352.png)
...
![image-20220715164718970](image/MySQL(5)日志与备份篇.assets/image-20220715164718970.png)

![image-20220715164743351](image/MySQL(5)日志与备份篇.assets/image-20220715164743351.png)

![image-20220715164809401](image/MySQL(5)日志与备份篇.assets/image-20220715164809401.png)
...
```mysql
mysqlbinlog -v --base64-output=DECODE-ROWS "/var/lib/mysql/binlog.000003"
...
# 查看最后100行
mysqlbinlog --no-defaults --base64-output=decode-rows -vv "/var/lib/mysql/binlog.000003" |tail -100
# 根据position查找
mysqlbinlog --no-defaults --base64-output=decode-rows -vv "/var/lib/mysql/binlog.000003" |grep -A
20 '4939002'
```
...
![image-20220715165603879](image/MySQL(5)日志与备份篇.assets/image-20220715165603879.png)
...
* **Statement**（基于SQL语句的复制(statement-based replication, **SBR**)）（默认）
  ...
  - 需要进行全表扫描(WHERE 语句中没有使用到索引)的 UPDATE 时，需要比 ROW 请求更多的**行级锁** 

* **Row**（基于行的复制(row-based replication, **RBR**)）

  5.1.5版本的MySQL才开始支持row level 的复制，它**不记录sql**语句上下文相关信息，**仅保存哪条记录被修改为什么**。

  优点：

  - row level 的日志内容会非常清楚的记录下每一行数据修改的细节。
  - 而且不会出现某些特定情况下的存储过程，或function，以及trigger的调用和触发无法被正确复制的问题。
  - 以下几种语句时的**行锁**更少：**INSERT ... SELECT**、**包含 AUTO_INCREMENT 字段的 INSERT**、 **没有附带条件或者并没有修改很多记录的 UPDATE 或 DELETE 语句** 
...
![image-20230918192713714](image/MySQL(5)日志与备份篇.assets/image-20230918192713714.png)

```shell
/usr/bin/mysqlbinlog --start-position=2247 --stop-position=3226 --database=atguigudb3 /var/lib/mysql/binlog.000003 | /usr/bin/mysql -uroot -p123456 -v atguigudb3
```
...
```shell
mysqlbinlog --no-defaults --base64-output=decode-rows -vv "/var/lib/mysql/binlog.000003" | tail -300
```

> 改为自己的filename和想要的tail数据量

```shell
/usr/bin/mysqlbinlog --start-datetime="2023-09-16 14:57:38" --stop-datetime="2023-09-16 14:58:39" --database=atguigudb3 /var/lib/mysql/binlog.000003 | /usr/bin/mysql -uroot -p123456 -v atguigudb3
```
...
# at 854467540
#230916 14:57:38 server id 1  end_log_pos 854467621 CRC32 0xa73fc810 	Query	thread_id=58	exec_time=0	error_code=0
SET TIMESTAMP=1694847458/*!*/;
BEGIN
/*!*/;
# at 854467621
#230916 14:57:38 server id 1  end_log_pos 854467689 CRC32 0x40bb9b80 	Table_map: `atguigudb3`.`student` mapped to number 195
# at 854467689
#230916 14:57:38 server id 1  end_log_pos 854467737 CRC32 0x9c284b28 	Write_rows: table id 195 flags: STMT_END_F
### INSERT INTO `atguigudb3`.`student`
### SET
###   @1=21 /* INT meta=0 nullable=0 is_null=0 */
###   @2='AA' /* VARSTRING(60) meta=60 nullable=1 is_null=0 */
###   @3='NO.1' /* VARSTRING(30) meta=30 nullable=1 is_null=0 */
# at 854467737
#230916 14:57:38 server id 1  end_log_pos 854467768 CRC32 0xf3dd9041 	Xid = 218189253
COMMIT/*!*/;
# at 854467768
#230916 14:57:41 server id 1  end_log_pos 854467847 CRC32 0xee13b4b4 	Anonymous_GTID	last_committed=180	sequence_number=181	rbr_only=yes	original_committed_timestamp=1694847461607270	immediate_commit_timestamp=1694847461607270	transaction_length=307
/*!50718 SET TRANSACTION ISOLATION LEVEL READ COMMITTED*//*!*/;
# original_commit_timestamp=1694847461607270 (2023-09-16 14:57:41.607270 CST)
# immediate_commit_timestamp=1694847461607270 (2023-09-16 14:57:41.607270 CST)
/*!80001 SET @@session.original_commit_timestamp=1694847461607270*//*!*/;
...
SET @@SESSION.GTID_NEXT= 'ANONYMOUS'/*!*/;
# at 854468768
#230916 14:58:22 server id 1  end_log_pos 854468858 CRC32 0xaabd71e2 	Query	thread_id=58	exec_time=0	error_code=0
SET TIMESTAMP=1694847502/*!*/;
BEGIN
/*!*/;
# at 854468858
#230916 14:58:22 server id 1  end_log_pos 854468926 CRC32 0xc3ba182a 	Table_map: `atguigudb3`.`student` mapped to number 195
# at 854468926
#230916 14:58:22 server id 1  end_log_pos 854468988 CRC32 0x0bb221f5 	Update_rows: table id 195 flags: STMT_END_F
...
###   @3='NO.1' /* VARSTRING(30) meta=30 nullable=1 is_null=0 */
# at 854468988
#230916 14:58:22 server id 1  end_log_pos 854469019 CRC32 0x0d3c4273 	Xid = 218189258
COMMIT/*!*/;
# at 854469019
#230916 14:58:39 server id 1  end_log_pos 854469098 CRC32 0x3ddd90bd 	Anonymous_GTID	last_committed=184	sequence_number=185	rbr_only=yes	original_committed_timestamp=1694847519315715	immediate_commit_timestamp=1694847519315715	transaction_length=320
/*!50718 SET TRANSACTION ISOLATION LEVEL READ COMMITTED*//*!*/;
# original_commit_timestamp=1694847519315715 (2023-09-16 14:58:39.315715 CST)
# immediate_commit_timestamp=1694847519315715 (2023-09-16 14:58:39.315715 CST)
/*!80001 SET @@session.original_commit_timestamp=1694847519315715*//*!*/;
/*!80014 SET @@session.original_server_version=80025*//*!*/;
/*!80014 SET @@session.immediate_server_version=80025*//*!*/;
```
...
![image-20220715171712026](image/MySQL(5)日志与备份篇.assets/image-20220715171712026.png)

![image-20220715172015185](image/MySQL(5)日志与备份篇.assets/image-20220715172015185.png)

**2. RESET MASTER: 删除所有二进制日志文件**

![image-20220715172104967](image/MySQL(5)日志与备份篇.assets/image-20220715172104967.png)
...
![image-20220715172958729](image/MySQL(5)日志与备份篇.assets/image-20220715172958729.png)
...
为**0**的时候，表示**每次提交事务都只 write，由系统自行判断什么时候执行fsync**（相当于redo log的参数为**2**的情况）。虽然性能得到提升，但是机器宕机，page cache里面的 binglog 会丢失。如下图：

![image-20220715193749462](image/MySQL(5)日志与备份篇.assets/image-20220715193749462.png)

为了安全起见，可以设置为默认值 `1` ，表示每次**提交事务都会执行fsync**，就如同**redo log 默认刷盘流程**一样（redo log参数为1时）。 最后还有一种折中方式，可以**设置为N(N>1)**，表示每次提交事务都write，但**累积N个事务后才fsync**。

![image-20220715194624080](image/MySQL(5)日志与备份篇.assets/image-20220715194624080.png)
...
![image-20220715194959405](image/MySQL(5)日志与备份篇.assets/image-20220715194959405.png)
...
![image-20220715195016492](image/MySQL(5)日志与备份篇.assets/image-20220715195016492.png)

由于binlog没写完就异常，这时候binlog里面没有对应的修改记录。因此，之后**从库**用binlog日志恢复数据时，就会少这一次更新，恢复出来的这一行c值为0，而**原库**因为redo log日志恢复，这一行c的值是1，最终**数据不一致**。

![image-20220715195521986](image/MySQL(5)日志与备份篇.assets/image-20220715195521986.png)

为了解决两份日志之间的逻辑一致问题，InnoDB存储引擎使用**两阶段提交**方案。原理很简单，将`redo log的写入拆成了两个步骤`prepare和commit，这就是**两阶段提交**。

![image-20220715195635196](image/MySQL(5)日志与备份篇.assets/image-20220715195635196.png)

使用两阶段提交后，写入binlog时发生异常也不会有影响，因为MySQL根据redo log日志恢复数据时，发现redo log还处于prepare阶段，并且没有对应binlog日志，就会回滚该事务。

![image-20220715200248193](image/MySQL(5)日志与备份篇.assets/image-20220715200248193.png)

另一个场景，redo log设置commit阶段发生异常，那会不会回滚事务呢？

![image-20220715200321717](image/MySQL(5)日志与备份篇.assets/image-20220715200321717.png)
...
| ----- | :--- | --------------------------- | ------------------------------------------------------------ | ------------------------------------------------------- |
| redo  | on   | %innodb_log%                | redo log buffer(16M,[1M,4096M])<br/>redo log block(512B)<br/>innodb_log_files_in_group(2,<=100)<br/>innodb_log_file_size(48M,<=512G)<br/>**innodb_flush_log_at_trx_commit(1,[0,2])** |                                                         |
| undo  | on   | %innodb_undo%               | innodb_rollback_segments(128)<br/>innodb_undo_tablespaces(2) |                                                         |
| gener | off  | %general%                   |                                                              |                                                         |
| error | on   | %log_error%                 |                                                              |                                                         |
| slow  | off  | %slow_quer%<br/>%long_quer% | long_query_time(10s)                                         | mysqldumpslow                                           |
| bin   | on   | %log_bin%<br/>%binlog%      | binlog_expire_logs_seconds(2592000即30天)<br/>binlog_cache_size(32KB)<br/>max_binlog_size(1G,<=1G)<br/>log_bin_trust_function_creators(OFF)<br/>**sync_binlog(1,[0,N])** | mysqlbinlog<br/>show binlog events<br/>show binary logs |
...
| 索引      | 《索引篇》9. MySQL监控分析视图-sys schema<br/>trace命令 默认关闭<br/>explain<br/>slow log慢查询日志 |
...
![image-20220715202237535](image/MySQL(5)日志与备份篇.assets/image-20220715202237535.png)
...
![image-20220715212521601](image/MySQL(5)日志与备份篇.assets/image-20220715212521601.png)
...
![image-20220715214055057](image/MySQL(5)日志与备份篇.assets/image-20220715214055057.png)
...
![image-20220715215944767](image/MySQL(5)日志与备份篇.assets/image-20220715215944767.png)
...
![image-20220715220037213](image/MySQL(5)日志与备份篇.assets/image-20220715220037213.png)
...
![image-20220715220852836](image/MySQL(5)日志与备份篇.assets/image-20220715220852836.png)
...
格式1： `STATEMENT模式 `默认格式 （基于SQL语句的复制(statement-based replication, SBR)）

```properties
binlog_format=STATEMENT
```

每一条会修改数据的`sql`语句会记录到binlog中。这是默认的binlog格式。
...
5.1.5版本的MySQL才开始支持，不记录每条sql语句的上下文信息，仅`记录哪条数据被修改了，修改成什么样了`。
...
注意：如果使用的是**MySQL8**，需要如下的方式建立账户，并授权slave:
...
+---------------------+----------+----------------------+------------------+-------------------+
| atguigu-bin.000002  |     1136 | atguigu_master_slave |                  |                   |
+---------------------+----------+----------------------+------------------+-------------------+
```
...
```mysql
CHANGE MASTER TO MASTER_HOST='192.168.111.100',MASTER_USER='slave1',MASTER_PASSWORD='123456',MASTER_LOG_FILE='atguigu-bin.000002',MASTER_LOG_POS=1136;
```

![image-20220718140946747](image/MySQL(5)日志与备份篇.assets/image-20220718140946747.png)
...
![image-20220718141825228](image/MySQL(5)日志与备份篇.assets/image-20220718141825228.png)

如果报错：

![image-20220718141841862](image/MySQL(5)日志与备份篇.assets/image-20220718141841862.png)
...
![image-20220718141951374](image/MySQL(5)日志与备份篇.assets/image-20220718141951374.png)
...
![image-20220718142045114](image/MySQL(5)日志与备份篇.assets/image-20220718142045114.png)
...
* 如何重新配置主从
  ...
  ![image-20220718142549168](image/MySQL(5)日志与备份篇.assets/image-20220718142549168.png)
...
![image-20220718143705843](image/MySQL(5)日志与备份篇.assets/image-20220718143705843.png)

![image-20220718143716620](image/MySQL(5)日志与备份篇.assets/image-20220718143716620.png)
...
![image-20220718144051094](image/MySQL(5)日志与备份篇.assets/image-20220718144051094.png)
...
![image-20220718144341584](image/MySQL(5)日志与备份篇.assets/image-20220718144341584.png)

读写分离情况下，解决主从同步中数据不一致的问题， 就是解决主从之间 `数据复制方式` 的问题，如果按照数据一致性 `从弱到强` 来进行划分，有以下 3 种复制方式。

#### 方法 1：异步复制

异步模式就是客户端**提交 COMMIT 之后不需要等从库返回任何结果，而是直接将结果返回给客户端**，这样做的好处是不会影响主库写的效率，但**可能会存在主库宕机**，而Binlog还没有同步到从库的情况，也就是此时的主库和从库数据不一致。这时候从从库中选择一个作为新主，那么新主则可能缺少原来主服务器中已提交的事务。所以，这种复制模式下的数据一致性是最弱的。

![image-20220718144410731](image/MySQL(5)日志与备份篇.assets/image-20220718144410731.png)

#### 方法 2：半同步复制

![image-20220718144926758](image/MySQL(5)日志与备份篇.assets/image-20220718144926758.png)

![image-20220718144958357](image/MySQL(5)日志与备份篇.assets/image-20220718144958357.png)
...
首先我们将多个节点共同组成一个复制组，在 `执行读写（RW）事务` 的时候，需要通过**一致性协议层** （Consensus 层）的同意，也就是读写事务想要进行提交，必须要经过组里“大多数人”（对应 Node 节 点）的同意，大多数指的是同意的节点数量需要大于 （N/2+1），这样才可以进行提交，而不是原发起方一个说了算。而针对 `只读（RO）事务` 则不需要经过组内同意，直接 COMMIT 即可。

在一个复制组内有多个节点组成，它们各自维护了自己的数据副本，并且在一致性协议层实现了原子消 息和全局有序消息，从而保证组内数据的一致性。

![image-20220718145235499](image/MySQL(5)日志与备份篇.assets/image-20220718145235499.png)
...
![image-20220718145428456](image/MySQL(5)日志与备份篇.assets/image-20220718145428456.png)
...
![image-20220718145523643](image/MySQL(5)日志与备份篇.assets/image-20220718145523643.png)

![image-20220718145534856](image/MySQL(5)日志与备份篇.assets/image-20220718145534856.png)

主备切换：

![image-20220718145548526](image/MySQL(5)日志与备份篇.assets/image-20220718145548526.png)
...
![image-20220718145936444](image/MySQL(5)日志与备份篇.assets/image-20220718145936444.png)

## 1. 物理备份与逻辑备份

**物理备份：**备份数据文件，转储数据库物理文件到某一目录。物理备份恢复速度比较快，但占用空间比较大，MySQL中可以用 `xtrabackup` 工具来进行物理备份。因为InnoDB的表文件不能直接复制，所以**物理备份只适用于MySIAM**
...
```mysql
mysqldump -uroot -p123456 atguigudb3 > atguigudb3.sql #备份文件存储在当前目录下
```

```mysql
mysqldump -uroot -p123456 atguigudb3 > /var/lib/mysql/backup/atguigudb3.sql
```
...
mysqldump备份**默认是不包含存储过程，自定义函数及事件**的。可以使用 `--routines` 或 `-R` 选项来备份存储过程及函数，使用 `--events` 或 `-E` 参数来备份事件。
...
可能有这样的需求，比如说我们只想恢复某一个库，但是我们有的是整个实例的备份，这个时候我们可以**从全量备份中分离出单个库**的备份。
...
```mysql
cat atguigudb3.sql | sed -e '/./{H;$!d;}' -e 'x;/CREATE TABLE `student`/!d;q' > student_structure.sql
cat atguigudb3.sql | grep --ignore-case 'insert into `student`' > student_data.sql
...
[root@redis100 mysql] cp -r atguigudb05 ./backup/
...
```
[root@redis100 mysql] cp -r ./backup/atguigudb05 ./

mysql> chown -R mysql.mysql /var/lib/mysql/atguigudb5
```
...
![image-20220718163627669](image/MySQL(5)日志与备份篇.assets/image-20220718163627669.png)
...
![image-20220718165515965](image/MySQL(5)日志与备份篇.assets/image-20220718165515965.png)

## 8. 删库了不敢跑，能干点啥？

![image-20220718165738367](image/MySQL(5)日志与备份篇.assets/image-20220718165738367.png)

### 8.1 delete：误删行

![image-20220718165909464](image/MySQL(5)日志与备份篇.assets/image-20220718165909464.png)

### 8.2 truncate/drop ：误删库/表

![image-20220718170026929](image/MySQL(5)日志与备份篇.assets/image-20220718170026929.png)

### 8.3 预防使用truncate/drop误删库/表

![image-20220718170132339](image/MySQL(5)日志与备份篇.assets/image-20220718170132339.png)

CHANGE MASTERRR TO MASTER_DELAY = N

![image-20220718170215247](image/MySQL(5)日志与备份篇.assets/image-20220718170215247.png)
