...
![image-20220708124306444](image/MySQL(4)事务篇.assets/image-20220708124306444.png)
...
![image-20220708164610193](image/MySQL(4)事务篇.assets/image-20220708164610193.png)
...
* **失败的（failed）**
  ...
  当事务处在 `活动的` 或者 部分提交的状态时，可能遇到了某些错误（数据库自身的错误、操作系统错误或者直接断电等）而无法继续执行，或者人为的停止当前事务的执行，我们就说该事务处在失败的状态。
...
* **提交的（committed）**
  ...
  ![image-20220708171859055](image/MySQL(4)事务篇.assets/image-20220708171859055.png)
...
> 补充：只读事务中只是不允许修改那些其他事务也能访问到的表中的数据，对于临时表来说（我们使用 CREATE TMEPORARY TABLE 创建的表），由于它们只能再当前会话中可见，所以**只读事务其实也是可以对临时表进行增、删、改操作的**。
...
![image-20220708201221316](image/MySQL(4)事务篇.assets/image-20220708201221316.png)
...
![image-20220708214453902](image/MySQL(4)事务篇.assets/image-20220708214453902.png)
...
![image-20220708215109480](image/MySQL(4)事务篇.assets/image-20220708215109480.png)
...
![image-20220708215626435](image/MySQL(4)事务篇.assets/image-20220708215626435.png)
...
![image-20220708220102342](image/MySQL(4)事务篇.assets/image-20220708220102342.png)
...
![image-20220708220228436](image/MySQL(4)事务篇.assets/image-20220708220228436.png)

> 解决幻读的两种方法：①MVCC ②加锁（间隙锁）
>
>  
>
> 以下是描述幻读的典型情况：
>
> 1. 事务A在Repeatable Read隔离级别下执行了一个查询操作，得到了一些行的结果集。
> 2. 在事务A的查询操作完成之后，但事务A尚未提交或回滚时，事务B插入了新的记录，这些记录与事务A的查询条件匹配。
> 3. 如果事务A再次执行相同的查询操作，它将会看到第一次查询中不存在的新记录，从而导致了幻读。
>
> **这种幻读可以通过InnoDB的MVCC机制解决，在InnoDB RR下不会出现这种幻读**
>
>  
>
> 还有一种幻读的情况：
>
> 1. 在RR隔离级别下，事务A查询到某id没有记录值，准备进行插入，但这时事务B先插入了这个id的记录，并提交；
> 2. 事务A插入时发现主键冲突无法插入造成幻读
>
> **这种幻读在RR隔离级别下，需要通过手动添加间隙锁来解决幻读问题，或者将隔离级别换为SERIALIZABLE**
...
![image-20220708220917267](image/MySQL(4)事务篇.assets/image-20220708220917267.png)

`脏写 `怎么没涉及到？因为脏写这个问题太严重了，不论是哪种隔离级别，都不允许脏写的情况发生。

不同的隔离级别有不同的现象，并有不同的锁和并发机制，隔离级别越高，数据库的并发性能就越差，4 种事务隔离级别与并发性能的关系如下：

![image-20220708220957108](image/MySQL(4)事务篇.assets/image-20220708220957108.png)

### 3.4 MySQL支持的四种隔离级别

![image-20220708221639979](image/MySQL(4)事务篇.assets/image-20220708221639979.png)
...
![image-20220708223250773](image/MySQL(4)事务篇.assets/image-20220708223250773.png)

**演示1. 读未提交之脏读**

设置隔离级别为未提交读：

![image-20220710193920008](image/MySQL(4)事务篇.assets/image-20220710193920008.png)

脏读就是指当前事务就在访问数据，并且对数据进行了修改，而这种修改还没有提交到数据库中，这时，另外一个事务也访问了这个数据，然后使用了这个数据。

**演示2：读已提交**

![image-20220710194440101](image/MySQL(4)事务篇.assets/image-20220710194440101.png)

**演示3. 不可重复读**

设置隔离级别为可重复读，事务的执行流程如下：

![image-20220710194144826](image/MySQL(4)事务篇.assets/image-20220710194144826.png)

当我们将当前会话的隔离级别设置为可重复读的时候，当前会话可以重复读，就是每次读取的结果集都相同，而不管其他事务有没有提交。但是在可重复读的隔离级别上会出现幻读的问题。

**演示4：幻读**

![image-20220710194042096](image/MySQL(4)事务篇.assets/image-20220710194042096.png)

> 当事务隔离级别为**REPEATABLE-READ**时：因为可重复读的原因，明明查询到id为3没有数据，可是却无法插入
>
> 当事务隔离级别为**SERIALIZABLE**时：因为加锁的原因，只有当前事务先**进行了查询操作**，那么另一个事务就无法插入数据，会进行等待操作，只能等待前一个事务提交或回滚后才能继续执行；若当前事务并**没有执行过查询操作**，那么下一个事务可以进行插入，但是前一个事务再次查询时却能看到多了数据，不会发生幻读（自己测试结果，不一定正确）

![image-20220710194612317](image/MySQL(4)事务篇.assets/image-20220710194612317.png)
...
* undo log: 是存储引擎层 (innodb) 生成的日志，记录的是 `逻辑操作` 日志，比如对某一行数据进行了INSERT语句操作，那么undo log就记录**一条与之相反的DELETE操作**。主要用于 `事务的回滚` (undo log 记录的是每个修改操作的 `逆操作`) 和 `一致性非锁定读` (undo log 回滚行记录到某种特定的版本——MVCC，即多版本并发控制)。

## 1. redo日志

**只有InnoDB才有redo log**。InnoDB存储引擎是以`页为单位`来管理存储空间的。在真正访问页面之前，需要把在`磁盘上`的页缓存到内存中的`Buffer Pool`之后才可以访问。所有的变更都必须`先更新缓冲池`中的数据，然后缓冲池中的`脏页`会以一定的频率被刷入磁盘 (`checkPoint`机制)，通过缓冲池来优化CPU和磁盘之间的鸿沟，这样就可以保证整体的性能不会下降太快。
...
`另一个解决的思路` ：我们只是想让已经提交了的事务对数据库中数据所做的修改永久生效，即使后来系 统崩溃，在重启后也能把这种修改恢复出来。所以我们其实没有必要在每次事务提交时就把该事务在内 存中修改过的全部页面刷新到磁盘，只需要把 `修改` 了哪些东西 `记录一下` 就好。**比如，某个事务将系统表空间中第10号 页面中偏移量为 100 处的那个字节的值 1 改成 2** 。我们只需要记录一下：将第0号表 空间的10号页面的偏移量为100处的值更新为 2 
...
![image-20220710202517977](image/MySQL(4)事务篇.assets/image-20220710202517977.png)

> 即立马刷新数据页会有**修改量与刷新磁盘工作量严重不成比例** 和 **随机IO刷新较慢**两个缺点，
>
> 所以采取先简单记录一下更新的地方，写入一个redo log文件，速度很快
...
* **事务执行过程中，redo log不断记录**
  ...
  **redo log**跟**bin log**的区别，redo log是`存储引擎层`产生的，而bin log是`数据库层`产生的。假设一个事务，对表做10万行的记录插入，在这个过程中，一直不断的往redo log顺序记录，而bin log不会记录，直到这个事务提交，才会一次写入到bin log文件中。
...
> 因为磁盘一个扇区的大小为512字节，若写入的数据超过512B，那么这次写入将不是原子性的（详见1.7 3）

![image-20220710204114543](image/MySQL(4)事务篇.assets/image-20220710204114543.png)
...
![image-20220710204427616](image/MySQL(4)事务篇.assets/image-20220710204427616.png)

### 1.4 redo的整体流程

以一个更新事务为例，redo log 流转过程，如下图所示：

![image-20220710204810264](image/MySQL(4)事务篇.assets/image-20220710204810264-16574572910841.png)
...
![image-20220710205015302](image/MySQL(4)事务篇.assets/image-20220710205015302.png)
...
![image-20220710205948156](image/MySQL(4)事务篇.assets/image-20220710205948156.png)

另外，InnoDB存储引擎有一个后台线程，每隔`1秒`，就会把`redo log buffer`中的内容写到文件系统缓存(`page cache`)，然后调用刷盘操作。（<font color='red'>无论参数设置为多少都会在后台每隔一秒进行同步</font>）

![image-20220710210339724](image/MySQL(4)事务篇.assets/image-20220710210339724.png)

也就是说，一个没有提交事务的`redo log`记录，也可能会刷盘。因为在事务执行过程 redo log 记录是会写入 `redo log buffer`中，这些redo log 记录会被`后台线程`刷盘。

![image-20220710210532805](image/MySQL(4)事务篇.assets/image-20220710210532805.png)
...
![image-20220710210751414](image/MySQL(4)事务篇.assets/image-20220710210751414.png)

![image-20220710211318120](image/MySQL(4)事务篇.assets/image-20220710211318120.png)

![image-20220710211335379](image/MySQL(4)事务篇.assets/image-20220710211335379.png)

>小结innodb_flush_log_at_trx_commit=2
>为`2`时，只要事务提交成功，`redo log buffer`中的内容只写入文件系统缓存(`page cache`)。
>如果仅仅只是`MySQL`挂了不会有任何数据丢失，但是操作系统宕机可能会有`1`秒数据的丢失，这种情况下无
>法满足ACID中的D。但是数值2肯定是**效率适中**的。

![image-20220710211831675](image/MySQL(4)事务篇.assets/image-20220710211831675.png)
...
>数值0的话，是一种折中的做法，它的IO效率理论是**最高的**，这种策略也有丢失数据的风险，也无法保证D。
...
![image-20220710215001482](image/MySQL(4)事务篇.assets/image-20220710215001482.png)
...
![image-20220710215353893](image/MySQL(4)事务篇.assets/image-20220710215353893.png)

### 1.7 写入redo log buffer 过程 了解
...
![image-20220710220653131](image/MySQL(4)事务篇.assets/image-20220710220653131.png)

#### 2. redo 日志写入log buffer

![image-20220710220838744](image/MySQL(4)事务篇.assets/image-20220710220838744.png)

![image-20220710220919271](image/MySQL(4)事务篇.assets/image-20220710220919271.png)

![image-20220710221221981](image/MySQL(4)事务篇.assets/image-20220710221221981.png)

![image-20220710221318271](image/MySQL(4)事务篇.assets/image-20220710221318271.png)
...
![image-20220710221620291](image/MySQL(4)事务篇.assets/image-20220710221620291.png)
...
![image-20220710223117420](image/MySQL(4)事务篇.assets/image-20220710223117420.png)

![image-20220710223135374](image/MySQL(4)事务篇.assets/image-20220710223135374.png)

真正的redo日志都是存储到占用`496`字节大小的`log block body`中，图中的`log block header`和`log block trailer`存储的是一些管理信息。我们来看看这些所谓`管理信息`都有什么。

![image-20220711144546439](image/MySQL(4)事务篇.assets/image-20220711144546439.png)

![image-20220711144608223](image/MySQL(4)事务篇.assets/image-20220711144608223.png)
...
![image-20220711152137012](image/MySQL(4)事务篇.assets/image-20220711152137012.png)

![image-20220711152242300](image/MySQL(4)事务篇.assets/image-20220711152242300.png)
...
![image-20220711152631108](image/MySQL(4)事务篇.assets/image-20220711152631108.png)

如果 write pos 追上 checkpoint ，表示`日志文件组`满了，这时候不能再写入新的 redo log记录，MySQL 得 停下来，清空一些记录，把 checkpoint 推进一下。

![image-20220711152802294](image/MySQL(4)事务篇.assets/image-20220711152802294.png)

### 1.9 redo log 小结

![image-20220711152930911](image/MySQL(4)事务篇.assets/image-20220711152930911.png)

## 2. Undo日志

redo log是事务持久性的保证，undo log是事务**原子性**（A）的保证。在事务中 `更新数据` 的 `前置操作` 其实是要先写入一个 `undo log` 。
...
![image-20220711153523704](image/MySQL(4)事务篇.assets/image-20220711153523704.png)
...
![image-20220711153645204](image/MySQL(4)事务篇.assets/image-20220711153645204.png)

> 并不是物理上恢复之前的效果，知识逻辑、效果上看起来是之前的效果
...
> 大致理解：
>
> Innodb每一行数据都有一个回滚指针（InnoDB三个隐藏列中的回滚指针，详情看2.5 2），用于指向该行修改前的最后一个历史版本，这个历史版本存放在undo log中，如果要执行更新操作，会将原纪录放入undo log，并通过隐藏的回滚指针指向undo log中的原纪录。其他事务此时需要查询时，就是查询undo log中这行数据的最后一个历史版本。（详情看2.3 3.回滚段中的数据分类）
...
![image-20220711154936382](image/MySQL(4)事务篇.assets/image-20220711154936382.png)

第二个参数在mysql8种是**innodb_rollback_segments**



##### undo页的重用

![image-20220711155044078](image/MySQL(4)事务篇.assets/image-20220711155044078.png)
...
3. 在回滚段中，事务会不断填充盘区，直到事务结束或所有的空间被用完。如果当前的盘区不够用，事务会在段中请求扩展下一个盘区，如果所有已分配的盘区都被用完，事务会**覆盖最初的盘区**或者在回滚段**允许的情况下扩展新的盘区**来使用。

4. 回滚段存在于undo表空间中，在数据库中可以存在**多个**undo表空间，但**同一时刻**只能使用**一个** undo表空间。
...
5. 当事务提交时，InnoDB存储引擎会做以下两件事情：
   ...
   + 判断undo log所在的页是否可以重用（低于3/4），若可以分配给下个事务使用
...
事务提交后不能马上删除undo log及undo log所在的页。这是因为可能还有其他事务需要通过undo log来得到行记录之前的版本。故事务提交时将undo log放入一个链表中，是否可以最终删除undo log以undo log所在页由purge线程来判断，**因为可能有别的事务正在访问之前的版本**。
...
* update undo log
  ...
  update undo log记录的是对delete和update操作产生的undo log。该undo log可能需要提供**MVCC**机制，因此不能在事务提交时就进行删除。提交时放入undo log**链表**，等待purge线程决定清除的行为，然后才进行最后的删除。
...
![image-20220711162414928](image/MySQL(4)事务篇.assets/image-20220711162414928.png)

**只有Buffer Pool的流程：**

![image-20220711162505008](image/MySQL(4)事务篇.assets/image-20220711162505008.png)

**有了Redo Log和Undo Log之后：**

![image-20220711162642305](image/MySQL(4)事务篇.assets/image-20220711162642305.png)

在更新Buffer Pool中的数据之前，我们需要先将该数据事务开始之前的状态写入Undo Log中。假设更新到一半出错了，我们就可以通过Undo Log来回滚到事务开始前。

#### 2. 详细生成过程

![image-20220711162919157](image/MySQL(4)事务篇.assets/image-20220711162919157.png)
...
![image-20220711163725129](image/MySQL(4)事务篇.assets/image-20220711163725129.png)
...
![image-20220711164138414](image/MySQL(4)事务篇.assets/image-20220711164138414.png)
...
![image-20220711164421494](image/MySQL(4)事务篇.assets/image-20220711164421494.png)
...
* 针对于insert undo log
  ...
  因为**insert**操作的记录，只对事务本身可见，对其他事务不可见。故该undo log可以在**事务提交后直接删除**，不需要进行purge操作。

* 针对于update undo log

  **update和delete**的undo log可能需要提供MVCC机制，因此不能在事务提交时就进行删除。提交时**放入undo log链表**，等待purge线程**决定清除的行为**（可能有其它事务在访问undo链上的数据，删除的话是看`重用`还是`删除`），然后才进行最后的删除。
...
![image-20220711165612956](image/MySQL(4)事务篇.assets/image-20220711165612956.png)
...
`锁`是计算机协调多个进程或线程`并发访问某一资源`的机制。在程序开发中会存在多线程同步的问题，当多个线程并发访问某个数据的时候，尤其是针对一些敏感的数据（比如订单、金额等），我们就需要保证这个数据在任何时刻`最多只有一个线程`在访问，保证数据的`完整性`和`一致性`。在开发过程中加锁是为了保证数据的一致性，这个思相在据库领域中同样很重要。
...
![image-20220711181120639](image/MySQL(4)事务篇.assets/image-20220711181120639.png)

当一个事务想对这条记录做改动时，首先会看看内存中有没有与这条记录关联的 `锁结构` ，当没有的时候 就会在内存中生成一个 `锁结构` 与之关联。比如，事务` T1` 要对这条记录做改动，就需要生成一个 `锁结构` 与之关联：

![image-20220711192633239](image/MySQL(4)事务篇.assets/image-20220711192633239.png)
...
![image-20220711193732567](image/MySQL(4)事务篇.assets/image-20220711193732567.png)
...
![image-20220711194904328](image/MySQL(4)事务篇.assets/image-20220711194904328.png)
...
![image-20220711202206405](image/MySQL(4)事务篇.assets/image-20220711202206405.png)
...
![image-20220711203250284](image/MySQL(4)事务篇.assets/image-20220711203250284.png)
...
![image-20220711203519162](image/MySQL(4)事务篇.assets/image-20220711203519162.png)

### 3.1 从数据操作的类型划分：读锁、写锁

![image-20220711203723941](image/MySQL(4)事务篇.assets/image-20220711203723941.png)
...
![image-20220711204843684](image/MySQL(4)事务篇.assets/image-20220711204843684.png)

#### 1. 锁定读

![image-20220711212931912](image/MySQL(4)事务篇.assets/image-20220711212931912.png)

> for share 和 for update 不仅加上S锁和X锁，还会加上**意向锁**（具体见3.2 1 ②）

![image-20220711213741630](image/MySQL(4)事务篇.assets/image-20220711213741630.png)

```mysql
select * from t1 where c1 = 2 for update nowait/skip locked
```



![image-20220711214013208](image/MySQL(4)事务篇.assets/image-20220711214013208.png)



#### 2. 写操作

![image-20220711214412163](image/MySQL(4)事务篇.assets/image-20220711214412163.png)

> 1. 执行**insert**语句，对要操作的页加RW-X-LATCH，然后**判断是否有和插入意向锁冲突的锁(别的事务的间隙锁)**，如果**有**，加插入意向锁，进入锁等待，如果**没有**，直接写数据，不加任何锁(隐式锁)，结束后释放RW-X-LATCH；
> 2. 执行**select ... lock in share mode**语句，对要操作的页加RW-X-LATCH，如果页面上存在RW-X-LATCH会被阻塞，没有的话则判断记录上是否有活跃的事务，如果**存在**，则**为insert事务创建一个排他记录锁**，并将自己加入锁等待队列，最后也会释放RW-X-LATCH
>
> 详见3.4 1 隐式锁

>**innodb行级锁**是通过锁索引记录实现的，如果**更新**的列**没建索引是会锁住整个表**的。



### 3.2 从数据操作的粒度划分：表级锁、页级锁、行锁

![image-20220711214719510](image/MySQL(4)事务篇.assets/image-20220711214719510.png)

#### 1. 表锁（Table Lock）

![image-20220711214805088](image/MySQL(4)事务篇.assets/image-20220711214805088.png)

##### ① 表级别的S锁、X锁

一般情况下，**不会使用**InnoDB存储引擎提供的表级别的 `S锁` 和 `X锁` 。只会在一些特殊情况下，比方说 `崩溃恢复` 过程中用到。比如，在系统变量 `autocommit=0，innodb_table_locks = 1` 时， 手动获取 InnoDB存储引擎提供的表t 的 `S锁` 或者 `X锁` 可以这么写：
...
![image-20220711220342251](image/MySQL(4)事务篇.assets/image-20220711220342251.png)

或者

![image-20220711220418859](image/MySQL(4)事务篇.assets/image-20220711220418859.png)
...
![image-20220711220442269](image/MySQL(4)事务篇.assets/image-20220711220442269.png)
...
![image-20220711220502141](image/MySQL(4)事务篇.assets/image-20220711220502141.png)

步骤5：加读锁

我们为mylock表加read锁（读阻塞写），观察阻塞的情况，流程如下：

![image-20220711220553225](image/MySQL(4)事务篇.assets/image-20220711220553225.png)

![image-20220711220616537](image/MySQL(4)事务篇.assets/image-20220711220616537.png)

步骤6：加写锁

为mylock表加write锁，观察阻塞的情况，流程如下：

![image-20220711220711630](image/MySQL(4)事务篇.assets/image-20220711220711630.png)

![image-20220711220730112](image/MySQL(4)事务篇.assets/image-20220711220730112.png)

总结：

**MyISAM**在执行查询语句（SELECT）前，会给涉及的所有表加读锁，在执行增删改操作前，会给涉及的表加写锁。**InnoDB存储引擎是不会为这个表添加表级别的读锁和写锁**的。
...
* 表独占写锁（Table Write Lock）
  ...
  ![image-20220711220929248](image/MySQL(4)事务篇.assets/image-20220711220929248.png)

##### ② 意向锁 （intention lock）

InnoDB 支持 `多粒度锁（multiple granularity locking）` ，它允许 `行级锁` 与 `表级锁` 共存，而`意向锁`就是其中的一种 `表锁` 。

1. 意向锁的存在是为了**协调行锁和表锁的关系**，支持多粒度（表锁和行锁）的锁并存。
...
![image-20220711222132300](image/MySQL(4)事务篇.assets/image-20220711222132300.png)
...
![image-20220712124209006](image/MySQL(4)事务篇.assets/image-20220712124209006.png)

> 意向锁之间是**互相兼容**的:
>
> 因为意向锁是由行级锁产生的，行行之间本就没有关联，若是行号相等，互斥的也是行级锁而不是意向锁
>
>  
>
> 与意向锁互斥的是**表锁**：
>
> 因为意向锁是表级锁，是为了示意该表中已经有某些行上了行锁（行X或S锁）
...
**意向锁不会与行级的 共享 / 排他锁互斥**！正因为如此，意向锁并不会影响到多个事务对不同数据行加排他锁时的并发性。（不然我们直接用普通的表锁就行了）
...
2. **意向锁之间互不排斥**，但除了 IS 与 S 兼容外， `意向锁会与 表级共享锁 / 排他锁 互斥` 。 
...
这些是“Simple inserts”语句但是指定**部分新行**的自动递增值。例如 `INSERT INTO teacher (id,name) VALUES (1,'a'), (NULL,'b'), (5,'c'), (NULL,'d');` 只是指定了部分id的值。另一种类型的“混合模式插入”是 `INSERT ... ON DUPLICATE KEY UPDATE` 。

![image-20220712175552985](image/MySQL(4)事务篇.assets/image-20220712175552985.png)
...
`（3）innodb_autoinc_lock下·_mode = 2(“交错”锁定模式)`

从 MySQL 8.0 开始，交错锁模式是 `默认` 设置。

在这种锁定模式下，所有类INSERT语句都不会使用表级AUTO-INC锁，并且可以同时执行多个语句。这是最快和最可扩展的锁定模式，但是当使用基于语句的**复制或恢复**方案时，**从二进制日志bin log重播SQL语句进行主备复制时，这是不安全的会出现主从不一致的问题(所以需要将binlog格式设置为row，出现问题的示例具体可查看《MySQL(6)实战45讲笔记》39讲 > 自增锁的优化)。**
...
> **补充知识：《MySQL实战45讲第39讲》内容：**
>
> 1. 自增值存储位置：
>    - **MyISAM**引擎的自增值**保存在数据文件**中。
>    - **InnoDB**引擎的自增值，其实是**保存在了内存**中，MySQL8之前，重启后从表数据中找max(id)+1作为自增值，所以可能发生自增值的重用；MySQL8之后，自增值记录在redo log中
> 2. 自增值修改机制：
>    - 若手动插入了一个id大小大于自增值，那么自增值将会设置为以初始值开始，以设定的步长增长到大于插入值的第一个值
>    - 表的**自增 id** 达到上限后，**再申请时它的值就不会改变**，进而导致继续插入数据时报主键冲突的错误。
> 3. 为什么自增主键id不能保证是连续的？
>    - 唯一键冲突后，自增值不会回滚
>    - 事务回滚后，自增值不会回滚
>    - 批量插入自增锁的优化机制会连续申请id，同一个语句去申请自增id，每次申请到的自增id个数都是上一次的两倍。
> 4. 回顾一下binlog_format=row的优点：
>    - 无索引update操作时更少的行锁
>    - 解决了使用statement/mixed格式时产生的主从不一致问题
>    - 便于使用binlog恢复数据
>    - 可以将innodb_autoinc_lock_mode设置为2，提高并发性能
...
读锁之间不互斥，因此你可以有多个线程同时对一张表增删查改。读写锁之间、写锁之间都是互斥的，用来保证变更表结构操作的安全性，解决了DML、DDL和DDL操作之间的一致性问题。`不需要显式使用`，在访问一个表的时候会被自动加上。
...
![image-20220713142808924](image/MySQL(4)事务篇.assets/image-20220713142808924.png)

通过会话C可以看出会话B被阻塞，这是由于会话A拿到了teacher表的元数据读锁，会话B想申请teacher表的元数据写锁，由于读写锁互斥，会话B需要等待会话A释放元数据锁才能执行。

![image-20220713143156759](image/MySQL(4)事务篇.assets/image-20220713143156759.png)



> 在InnoDB存储引擎下，一个**事务**中的SQL语句"SELECT xxx FOR SHARE/UPDATE"会添加一个行级锁，并且可能会添加意向锁（Intention Lock），间接自动添加元数据读锁（Metadata Lock）。
...
> 表锁是MySQL最基本的锁策略，不依赖于存储引擎

**优点：**锁定力度小，发生`锁冲突概率低`，可以实现的`并发度高`。

**缺点：**对于`锁的开销比较大`，加锁会比较慢，容易出现`死锁`情况。

InnoDB与MyISAM的最大不同有两点：一是**支持事务**（TRANSACTION）；二是采用了**行级锁**。
...
![image-20220713161549241](image/MySQL(4)事务篇.assets/image-20220713161549241.png)

student表中的聚簇索引的简图如下所示。

![image-20220713163353648](image/MySQL(4)事务篇.assets/image-20220713163353648.png)
...
![image-20220713164811567](image/MySQL(4)事务篇.assets/image-20220713164811567.png)

举例如下：

![image-20220713164948405](image/MySQL(4)事务篇.assets/image-20220713164948405.png)
...
`MySQL` 在 `REPEATABLE READ` 隔离级别下是可以解决**幻读问题的，解决方案有两种，可以使用 `MVCC` 方 案解决，也可以采用 `加锁 `方案解决**。但是在使用加锁方案解决时有个大问题，就是事务在第一次执行读取操作时，那些幻影记录尚不存在，我们无法给这些 `幻影记录` 加上 `记录锁` 。InnoDB提出了一种称之为 `Gap Locks` 的锁，官方的类型名称为：` LOCK_GAP` ，我们可以简称为 `gap锁` 。比如，把id值为8的那条 记录加一个gap锁的示意图如下。

![image-20220713171650888](image/MySQL(4)事务篇.assets/image-20220713171650888.png)
...
**间隙锁不互斥**：虽然有`共享gap锁`和`独占gap锁`这样的说法，但是它们起到的作用是相同的。而且如果对一条记录加了gap锁（不论是共享gap锁还是独占gap锁），**并不会限制其他事务对这条记录加记录锁或者继续加gap锁**。
...
> 若使用select xxx where id = xxx **for update** / **for share** 查询语句，若是**存在的记录**，将会给对应的行加上**行锁（记录锁）**，若是**对应的行不存在**，则会加上范围为紧邻两条数据之间（例如(3,8)）的**间隙锁**，导致其他事务操作时(3,8)之间的id都不能添加成功，若添加的间隙锁大于最大记录，则间隙锁的范围为**(最大记录,+∞)**

![image-20220713174726264](image/MySQL(4)事务篇.assets/image-20220713174726264.png)
...
![image-20220713174108634](image/MySQL(4)事务篇.assets/image-20220713174108634.png)
...
![image-20220713174551814](image/MySQL(4)事务篇.assets/image-20220713174551814.png)

![image-20220713174602102](image/MySQL(4)事务篇.assets/image-20220713174602102.png)



**但是间隙锁容易引发死锁问题**

![image-20220713175032619](image/MySQL(4)事务篇.assets/image-20220713175032619.png)

![image-20220713192418730](image/MySQL(4)事务篇.assets/image-20220713192418730.png)

> 因为**间隙锁不互斥**，两个事务都能对同一范围添加了间隙锁，那么两个事务的添加操作都会阻塞，引发死锁问题
>
> MySQL的解决：①超时机制	②一方主动回滚	（详见3.6）
...
![image-20220713192549340](image/MySQL(4)事务篇.assets/image-20220713192549340.png)

`next-key锁`的本质就是**一个`记录锁`和一个`gap锁`的合体**，它既能保护该条记录，又能阻止别的事务将新记录插入被保护记录前边的`间隙`。
...
![image-20220713203124889](image/MySQL(4)事务篇.assets/image-20220713203124889.png)

> 插入意向锁不是意向锁，而是属于**间隙锁**，是一种行锁，主要是在插入insert时自动产生
>
> 插入意向锁是一种间隙锁形式的意向锁，在真正执行 INSERT 操作之前设置。
> 当执行插入操作时，总会检查当前插入操作的下一条记录（已存在的主索引节点）上是否存在gap锁对象，判断是否锁住了 gap，如果锁住了，则判定和插入意向锁冲突，当前插入操作就需要**等待**，也就是配合上面的间隙锁或者临键锁一起防止了幻读操作。
>
> 插入意向锁之间不会互相冲突，多个插入操作同时插入同一个 gap 时，无需互相等待，比如当前索引上有记录 4 和 8，两个并发 session 同时插入记录 6，7。他们会分别为(4,8)加上 GAP 锁，但相互之间并不冲突
>
>  
>
> 插入意向锁主要起一个**等待的作用**，若是多个插入意向锁在同一间隙进行锁等待，如果索引等因素上没有冲突，两个事务将会并发插入成功，若是有冲突，则为先排队的那个事务插入执行成功后上锁

> 1. 执行**insert**语句，对要操作的页加RW-X-LATCH，然后**判断是否有和插入意向锁冲突的锁(别的事务的间隙锁)**，如果**有**，加插入意向锁，进入锁等待，如果**没有**，直接写数据，不加任何锁(隐式锁)，结束后释放RW-XLATCH；
> 2. 执行**select ... lock in share mode**语句，对要操作的页加RW-X-LATCH，如果页面上存在RW-X-LATCH会被阻塞，没有的话则判断记录上是否有活跃的事务，如果**存在**，则**为insert事务创建一个排他记录锁**，并将自己加入锁等待队列，最后也会释放RW-X-LATCH
>
> 详见3.4 1 隐式锁

![image-20220713203532124](image/MySQL(4)事务篇.assets/image-20220713203532124.png)

![image-20220713203619704](image/MySQL(4)事务篇.assets/image-20220713203619704.png)

![image-20220713203714577](image/MySQL(4)事务篇.assets/image-20220713203714577.png)
...
![image-20220713204544767](image/MySQL(4)事务篇.assets/image-20220713204544767.png)

![image-20220713205010502](image/MySQL(4)事务篇.assets/image-20220713205010502.png)

![image-20220713205135694](image/MySQL(4)事务篇.assets/image-20220713205135694.png)
...
![image-20220713210951100](image/MySQL(4)事务篇.assets/image-20220713210951100.png)

![image-20220713211139670](image/MySQL(4)事务篇.assets/image-20220713211139670.png)
...
![image-20220713211417909](image/MySQL(4)事务篇.assets/image-20220713211417909.png)

### 3.4 按加锁的方式划分：显式锁、隐式锁

#### 1. 隐式锁

![image-20220713211525845](image/MySQL(4)事务篇.assets/image-20220713211525845.png)

* **情景一**：对于聚簇索引记录来说，有一个 `trx_id` 隐藏列，该隐藏列记录着最后改动该记录的 `事务 id` 。那么如果在当前事务中新插入一条聚簇索引记录后，该记录的 trx_id 隐藏列代表的的就是当前事务的事务id ，如果其他事务此时想对该记录添加 S锁 或者 X锁 时，**首先会看一下该记录的 trx_id 隐藏列代表的事务是否是当前的活跃事务**，如果**是**的话，那么就**帮助当前事务创建一个 X 锁** （也就是为当前事务创建一个锁结构， is_waiting 属性是 false ），然后**自己进入等待状态** （也就是为自己也创建一个锁结构， is_waiting 属性是 true ）。
* **情景二**：对于二级索引记录来说，本身并没有 trx_id 隐藏列，但是在二级索引页面的 Page Header 部分有一个 `PAGE_MAX_TRX_ID` 属性，该属性代表对该页面做改动的最大的 `事务id` ，如果 PAGE_MAX_TRX_ID 属性值**小于**当前最小的活跃事务id ，那么说明对该页面做修改的事务都已经提交了，否则就需要在页面中定位到对应的二级索引记录，然后回表找到它对应的聚簇索引记 录，然后再重复 `情景一` 的做法。

![image-20220713214522709](image/MySQL(4)事务篇.assets/image-20220713214522709.png)
...
![image-20220713220714098](image/MySQL(4)事务篇.assets/image-20220713220714098.png)

**举例2：**

用户A给用户B转账100，再次同时，用户B也给用户A转账100。这个过程，可能导致死锁。

![image-20220713220936236](image/MySQL(4)事务篇.assets/image-20220713220936236.png)
...
insert into student (id,name) values (5,'zx');       insert into student (id,name) values (6,'xm');
```
...
![image-20220713221418100](image/MySQL(4)事务篇.assets/image-20220713221418100.png)

**方式2：**使用死锁检测处理死锁程序（innodb_deadlock_detect=on）
...
![image-20220713221758941](image/MySQL(4)事务篇.assets/image-20220713221758941.png)

基于这两个信息，可以绘制wait-for graph（等待图）

![image-20220713221830455](image/MySQL(4)事务篇.assets/image-20220713221830455.png)

> 死锁检测的原理是构建一个以事务为顶点，锁为边的有向图，判断有向图**是否存在环**，存在既有死锁。
...
>死锁检测详细请查看《MySQL06实战45讲笔记》 07节
>
>**如何解决热点行更新导致的性能问题？**
>1、如果你能确保这个业务一定不会出现死锁，可以临时把死锁检测关闭掉。一般不建议采用
>2、控制并发度，对应相同行的更新，在进入引擎之前排队。这样在InnoDB内部就不会有大量的死锁检测工作了。
>3、将热更新的行数据拆分成逻辑上的多行来减少锁冲突（比如总金额等于各行之和），但是业务复杂度可能会大大提高。
>
...
![image-20220714131008260](image/MySQL(4)事务篇.assets/image-20220714131008260.png)
...
![image-20220714132306208](image/MySQL(4)事务篇.assets/image-20220714132306208.png)
...
![image-20220714133319666](image/MySQL(4)事务篇.assets/image-20220714133319666.png)
...
**MySQL8.0**删除了information_schema.INNODB_LOCKS，添加了 **`performance_schema.data_locks`** ，可以通过performance_schema.data_locks查看事务的锁情况，和MySQL5.7及之前不同， performance_schema.data_locks不但可以看到阻塞该事务的锁，还可以看到该事务所持有的锁。
...
## 6. MySQL的加锁规则（邻间锁的范围和规则）

**间隙锁加锁规则（共11个案例）**

间隙锁是在可重复读隔离级别下才会生效的： next-key lock 实际上是由间隙锁加行锁实现的，如果切换 到读提交隔离级别 (read-committed) 的话，就好理解了，过程中去掉间隙锁的部分，也就是只剩下行锁 的部分。而在读提交隔离级别下间隙锁就没有了，为了解决可能出现的数据和日志不一致问题，需要把 binlog 格式设置为 row 。也就是说，许多公司的配置为：读提交隔离级别加 binlog_format=row。业务不需要可重复读的保证，这样考虑到读提交下操作数据的锁范围更小（没有间隙锁），这个选择是合理的。

next-key lock的加锁规则

总结的加锁规则里面，包含了两个 “ “ 原则 ” ” 、两个 “ “ 优化 ” ” 和一个 “bug” 。

> 规则符合5.x系列<=5.7.24，8.0系列 <=8.0.13。

1. 原则1 ：加锁的基本单位是 next-key lock 。 next-key lock 是前开后闭区间。 
2. 原则2 ：查找过程中访问到的对象才会加锁。任何辅助索引上的锁，或者非索引列上的锁，若需要回表要回溯到主键上，在主键上也要加一把锁。 
3. 优化1：索引上的**等值**查询，给**唯一索引**加锁的时候，next-key lock**退化为行锁**。也就是说如果 InnoDB扫描的是一个主键、或是一个唯一索引的话，那InnoDB只会采用行锁方式来加锁 
4. 优化2：索引上的**等值**查询，向右遍历时且最后一个值不满足等值条件的时候，next-keylock **退化为间隙锁**。 
...
![image-20220714134603698](image/MySQL(4)事务篇.assets/image-20220714134603698.png)
...
![image-20220714134623052](image/MySQL(4)事务篇.assets/image-20220714134623052-16577775838551.png)
...
![image-20220714134742049](image/MySQL(4)事务篇.assets/image-20220714134742049.png)
...
![image-20220714134822160](image/MySQL(4)事务篇.assets/image-20220714134822160.png)
...
![image-20220714134846740](image/MySQL(4)事务篇.assets/image-20220714134846740.png)
...
![image-20220714134923414](image/MySQL(4)事务篇.assets/image-20220714134923414.png)
...
![image-20220714134945012](image/MySQL(4)事务篇.assets/image-20220714134945012.png)
...
![image-20220714135007118](image/MySQL(4)事务篇.assets/image-20220714135007118.png)
...
![image-20220714135025045](image/MySQL(4)事务篇.assets/image-20220714135025045-16577778257713.png)
...
![image-20220714135047760](image/MySQL(4)事务篇.assets/image-20220714135047760.png)
...
![image-20220714135130668](image/MySQL(4)事务篇.assets/image-20220714135130668.png)
...
![image-20220714135206504](image/MySQL(4)事务篇.assets/image-20220714135206504.png)
...
![image-20220714135300189](image/MySQL(4)事务篇.assets/image-20220714135300189.png)
...
![image-20220714135333089](image/MySQL(4)事务篇.assets/image-20220714135333089.png)
...
> **<font color='red'>修改删除操作和加锁的select操作都属于当前读</font>**
...
![image-20220714140441064](image/MySQL(4)事务篇.assets/image-20220714140441064.png)

![image-20220714140510426](image/MySQL(4)事务篇.assets/image-20220714140510426.png)

![image-20220714140541555](image/MySQL(4)事务篇.assets/image-20220714140541555.png)
...
![image-20220714140716427](image/MySQL(4)事务篇.assets/image-20220714140716427.png)

假设插入该记录的`事务id`为`8`，那么此刻该条记录的示意图如下所示：

![image-20220714140801595](image/MySQL(4)事务篇.assets/image-20220714140801595.png)

> insert undo只在事务回滚时起作用，当事务提交后，该类型的undo日志就没用了，它占用的Undo Log Segment也会被系统回收（也就是该undo日志占用的Undo页面链表要么被重用，要么被释放）。

假设之后两个事务id分别为 `10` 、 `20` 的事务对这条记录进行` UPDATE` 操作，操作流程如下：

![image-20220714140846658](image/MySQL(4)事务篇.assets/image-20220714140846658.png)

![image-20220714140908661](image/MySQL(4)事务篇.assets/image-20220714140908661.png)

每次对记录进行改动，都会记录一条undo日志，每条undo日志也都有一个 `roll_pointer` 属性 （ `INSERT` 操作对应的undo日志没有该属性，因为该记录并没有更早的版本），可以将这些 `undo日志` 都连起来，串成一个链表：

![image-20220714141012874](image/MySQL(4)事务篇.assets/image-20220714141012874.png)
...
![image-20220714141154235](image/MySQL(4)事务篇.assets/image-20220714141154235.png)
...
1. `creator_trx_id` ，创建这个 Read View 的事务 ID。
   ...
   > 说明：只有在对表中的记录做改动时（执行INSERT、DELETE、UPDATE这些语句时）才会为事务分配事务id，否则在**一个只读事务中的事务id值都默认为0。**

2. `trx_ids` ，表示在生成ReadView时当前系统中活跃的读写事务的 `事务id列表` 。

3. `up_limit_id` ，活跃的事务中最小的事务 ID。

4. `low_limit_id` ，表示生成ReadView时系统中应该**分配给下一个事务**的 `id` 值。即系统最大事务id + 1，low_limit_id 是系统最大的事务id值，这里要注意是系统中的事务id，需要区别于正在活跃的事务ID。
...
![image-20220714142254768](image/MySQL(4)事务篇.assets/image-20220714142254768.png)



### 4.3 ReadView的规则

有了这个ReadView，这样某个`事务`在访问某条`记录`时，只需要按照下边的步骤判断**记录的某个版本**是否可见。

* 如果被访问版本的trx_id属性值与ReadView中的 `creator_trx_id` 值相同，意味着**当前事务在访问它自己修改过的记录**，所以该版本**可以**被当前事务访问。 
* 如果被访问版本的trx_id属性值小于ReadView中的 `up_limit_id` 值，表明生成该版本的事务在**当前事务生成ReadView前已经提交**，所以该版本**可以**被当前事务访问。 
* 如果被访问版本的trx_id属性值大于或等于ReadView中的 `low_limit_id` 值，表明生成该版本的事务在**当前事务生成ReadView后才开启**，所以该版本**不可以**被当前事务访问。 
* 如果被访问版本的trx_id属性值在ReadView的 `up_limit_id` 和 `low_limit_id` 之间，那就需要判断一下trx_id属性值是不是在 `trx_ids` 列表中。
  * 如果**在**，说明**创建ReadView时生成该版本的事务还是活跃**的，该版本**不可以**被访问。 
  * 如果**不在**，说明**创建ReadView时生成该版本的事务已经被提交**，该版本**可以**被访问。



>某个事务访问某条记录时，查询这条记录的版本号：
>
>一共只有三种情况表示能访问到记录当前版本：只有当<font color='orange'>①记录版本的trx_id等于事务创建read view的creator_tx_id</font>或者<font color='orange'>②记录版本的trx_id小于read view的up_limit_id</font>或者<font color='orange'>③不在活跃的trx+ids当中</font>时才表示可以被当前事务访问到此纪录的当前版本
>
>其它时候，如在read view活跃的trx_ids当中或者大于等于low_limit_id，都只能<font color='cornflowerblue'>沿着undo log链表往前查找自己能够访问的版本</font>
...
![image-20220715130639408](image/MySQL(4)事务篇.assets/image-20220715130639408.png)
...
![image-20220715130843147](image/MySQL(4)事务篇.assets/image-20220715130843147.png)
...
![image-20220715130916437](image/MySQL(4)事务篇.assets/image-20220715130916437.png)

## 5. 举例说明

![image-20220715131200077](image/MySQL(4)事务篇.assets/image-20220715131200077.png)
...
> 说明：事务执行过程中，**select操作的事务id会是默认值0**，只有在第一次真正修改**记录时（比如使用INSERT、DELETE、UPDATE语句），才会被分配一个单独的事务id**，这个事务id是递增的。所以我们才在事务2中更新一些别的表的记录，目的是让它分配事务id。

此刻，表student 中 id 为 1 的记录得到的版本链表如下所示：

![image-20220715133640655](image/MySQL(4)事务篇.assets/image-20220715133640655.png)
...
![image-20220715134540737](image/MySQL(4)事务篇.assets/image-20220715134540737.png)
...
![image-20220715134839081](image/MySQL(4)事务篇.assets/image-20220715134839081.png)
...
![image-20220715135017000](image/MySQL(4)事务篇.assets/image-20220715135017000.png)

![image-20220715135143939](image/MySQL(4)事务篇.assets/image-20220715135143939.png)
...
![image-20220715140006061](image/MySQL(4)事务篇.assets/image-20220715140006061.png)
...
![image-20220715140155744](image/MySQL(4)事务篇.assets/image-20220715140155744.png)
...
![image-20220715140354217](image/MySQL(4)事务篇.assets/image-20220715140354217.png)
...
![image-20220715140555172](image/MySQL(4)事务篇.assets/image-20220715140555172.png)

![image-20220715140620328](image/MySQL(4)事务篇.assets/image-20220715140620328.png)
...
![image-20230922150310865](image/MySQL(4)事务篇.assets/image-20230922150310865.png)

事务B读出的值为(1,3)

但是：**事务B的update语句，如果按照一致性读，好像结果不对哦？**
...
READ VIEW分为快照读和当前读，**<font color='red'>修改删除操作和加锁的select操作都属于当前读</font>**
...
![image-20220715141002035](image/MySQL(4)事务篇.assets/image-20220715141002035.png)
...
结论：事务 A 的第一次查询，**目前只能读取到一条数据**，id=1。
...
![image-20220715141208667](image/MySQL(4)事务篇.assets/image-20220715141208667.png)
...
1）首先 id=1 的这条数据，前面已经说过了，**可以**被事务 A 看到。 

2）然后是 id=2 的数据，它的 trx_id=30，此时事务 A 发现，这个值处于 up_limit_id 和 low_limit_id 之 间，因此还需要再判断 30 是否处于 trx_ids 数组内。由于事务 A 的 trx_ids=[20,30]，因此在数组内，这表 示 id=2 的这条数据是与事务 A 在同一时刻启动的其他事务提交的，所以这条数据**不能**让事务 A 看到。

3）同理，id=3 的这条数据，trx_id 也为 30，因此也**不能**被事务 A 看见。

![image-20220715141243993](image/MySQL(4)事务篇.assets/image-20220715141243993.png)
...
![image-20220715141413135](image/MySQL(4)事务篇.assets/image-20220715141413135.png)

通过MVCC我们可以解决：

![image-20220715141515370](image/MySQL(4)事务篇.assets/image-20220715141515370.png)